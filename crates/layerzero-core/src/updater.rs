...
const BINARIES: &[&str] = &["layerzero", "layerzero-server", "layerzero-mcp"];
...
async fn fetch_latest(repo: &str) -> Result<GithubRelease> {
    let client = Client::builder()
        .user_agent(format!("layerzero/{}", current_version()))
        .build()?;
...
pub async fn update_now(cfg: &UpdateConfig) -> Result<UpdateOutcome> {
    ...
    let staging = std::env::temp_dir().join(format!("layerzero-update-{}", uuid::Uuid::new_v4()));
    std::fs::create_dir_all(&staging)?;
    let extracted = extract_binaries(&bytes, &asset.name, &staging)?;
    if extracted.is_empty() {
        return Err(anyhow!("update archive contained no layerzero binaries"));
    }
    ...
    for src in extracted {
        ...
        if Some(&name) == current_name.as_ref() {
            ...
            match std::fs::copy(src, &dest) {
                ...
                Err(e) => warn!(
                    "could not replace {} (in use? stop it and re-run `layerzero update`): {}",
                    dest.display(),
                    e
                ),
            }
