...
use anyhow::Result;
use clap::Parser;
use layerzero_core::config::Config;
...
#[derive(Parser)]
#[command(name = "layerzero-mcp", about = "layerzero MCP server (stdio JSON-RPC 2.0)")]
struct Args {
    #[arg(short, long)]
...
async fn main() -> Result<()> {
    tracing_subscriber::registry()
        .with(EnvFilter::try_from_default_env().unwrap_or_else(|_| "layerzero=info".into()))
    ...
    tracing::info!("layerzero MCP server ready");
...
async fn handle(ctx: &ToolContext, req: JsonRpcRequest) -> JsonRpcResponse {
    ...
    match req.method.as_str() {
        "initialize" => JsonRpcResponse::ok(id, json!({
            ...
            "capabilities": { "tools": {} },
            "serverInfo": { "name": "layerzero", "version": env!("CARGO_PKG_VERSION") }
        })),
