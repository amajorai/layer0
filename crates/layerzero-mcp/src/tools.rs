...
pub async fn search_memory(ctx: &ToolContext, args: &Value) -> Result<Value> {
    ...
    let results = search_similar(&ctx.pool, &ctx.llm, query, &ctx.config.llm.embedding_model, limit, 0.0).await?;
...
pub async fn get_document_tool(ctx: &ToolContext, args: &Value) -> Result<Value> {
    ...
    let row: Option<Row> = sqlx::query_as(
        "SELECT id, content, metadata, source, created_at FROM documents WHERE id = ?"
    )
    .bind(id)
...
pub async fn graph_query_tool(ctx: &ToolContext, args: &Value) -> Result<Value> {
    ...
    let result = bfs_traverse(&ctx.pool, &start_id, depth, relation, "both").await?;
...
pub async fn memory_stats_tool(ctx: &ToolContext, _args: &Value) -> Result<Value> {
    let docs: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM documents").fetch_one(&ctx.pool).await?;
    let embs: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM embeddings").fetch_one(&ctx.pool).await?;
    let nodes: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM graph_nodes").fetch_one(&ctx.pool).await?;
