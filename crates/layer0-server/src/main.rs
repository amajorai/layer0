...
#[derive(Parser)]
#[command(name = "layer0-server", version, about = "layer0 RAG memory server")]
struct Args {
    #[arg(short, long)]
    config: Option<PathBuf>,
    #[arg(long, env = "LAYER0_HOST")]
    host: Option<String>,
    #[arg(short, long, env = "LAYER0_PORT")]
    port: Option<u16>,
    #[arg(long, env = "LAYER0_LLM_BASE_URL")]
...
async fn main() -> Result<()> {
    ...
    if config.update.auto_update {
        match layer0_core::updater::update_now(&config.update).await {
            Ok(layer0_core::updater::UpdateOutcome::Updated { from, to }) => {
                tracing::warn!("updated layer0 {} -> {} (restart to apply)", from, to)
            }
            Ok(_) => {}
    ...
    if !llm.health().await {
        tracing::warn!(
            "Embeddings backend not reachable at {}. Start llama-server or set LAYER0_LLM_BASE_URL.",
            config.llm.base_url
        );
    } else {
    ...
    info!("layer0 server listening on http://{}", addr);
