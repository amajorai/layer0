---
name: layerzero-memory
description: >-
  Use a running layerzero server as long-term, persistent memory for an AI
...
metadata:
  version: "0.0.1"
compatibility: >-
  Requires a running layerzero server (see the layerzero-setup skill). MCP tools
  require the layerzero MCP server connected to your agent; HTTP calls require
  the API reachable at http://127.0.0.1:8080 (default).
---

# Using layerzero as long-term memory

layerzero is a self-hostable RAG memory server. Once it is running (see the
`layerzero-setup` skill), use it to persist knowledge across sessions and to
...
If the layerzero MCP server is connected to your agent, these tools are
available:
...
layerzero scopes memory with **databases** and **collections**. Use them to keep
...
- A **collection** groups related memories within a database (e.g.
  `preferences`, `project-layerzero`, `meeting-notes`).
